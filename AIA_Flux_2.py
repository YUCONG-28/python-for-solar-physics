...
data_file = os.path.join(output_dir, 'aia_1600_selected_2.csv')  # 数据存储路径
...
def process_fits_files():
    ...
    try:
        ...
        target_regions_sky = [
            (SkyCoord(Tx=310*u.arcsec, Ty=-200*u.arcsec, frame=temp_map.coordinate_frame),
             SkyCoord(Tx=420*u.arcsec, Ty=-140*u.arcsec, frame=temp_map.coordinate_frame)), 
            
            (SkyCoord(Tx=310*u.arcsec, Ty=-250*u.arcsec, frame=temp_map.coordinate_frame),
             SkyCoord(Tx=420*u.arcsec, Ty=-200*u.arcsec, frame=temp_map.coordinate_frame)), 
        ]
