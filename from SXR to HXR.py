...
# 配置参数（可根据需求调整）
# --------------------------
DATA_FILE_PATH = 'd:/Flare/dn_xrsf-l2-flx1s_g16_d20240808_v2-2-0.nc'
...
# 优先选择支持完整符号集的字体
FONT_CANDIDATES = ["Arial", "Times New Roman", "SimHei", "Microsoft YaHei", "DejaVu Sans"]
...
def smooth_flux_data(flux_data, window_length, polyorder):
    ...
    window_length = min(window_length, len(flux_data) if len(flux_data) % 2 == 1 else len(flux_data) - 1)
...
def visualize_results(time, xrsa_raw, xrsa_smooth, xrsb_raw, xrsb_smooth, xrsa_deriv, xrsb_deriv):
    fig, axes = plt.subplots(4, 1, figsize=FIG_SIZE, sharex=True)
    
    plot_flux_comparison(
        axes[0], time, xrsa_raw, xrsa_smooth,
        "GOES-16 太阳软X射线流量 (0.5-4 Angstrom)",
        "流量 (W/m²)"
    )
    
    plot_flux_comparison(
        axes[1], time, xrsb_raw, xrsb_smooth,
        "GOES-16 太阳软X射线流量 (1.0-8 Angstrom)",
        "流量 (W/m²)"
    )
    
    plot_derivative(
        axes[2], time, xrsa_deriv,
        "GOES-16 太阳软X射线流量导数 (0.5-4 Angstrom)",
        "流量 (W/(m²s)"
    )
    
    plot_derivative(
        axes[3], time, xrsb_deriv,
...
def main():
    try:
        ...
        visualize_results(
            time_subset, xrsa_flux, xrsa_smoothed,
            xrsb_flux, xrsb_smoothed,
            xrsa_derivative, xrsb_derivative
        )
