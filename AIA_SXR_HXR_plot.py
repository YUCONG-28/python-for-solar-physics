...
def load_sxr_data(file_path, start_time, end_time):
    ...
    # 转换时间格式
    times = [datetime.utcfromtimestamp(t.astype("datetime64[ns]").astype("int64") / 1e9) 
...
def load_aia_data(dir_path, file_pattern, start_time, end_time):
    ...
    all_files = [f for f in os.listdir(dir_path) 
                ...
                and f.endswith('.csv')
                and fnmatch.fnmatch(f, file_pattern)]  # 按模式匹配文件名
    all_files.sort()
    
    if not all_files:
        print(f"警告：文件夹 '{dir_path}' 中没有找到匹配模式 '{file_pattern}' 的CSV文件")
        return []
    
    print(f"已选择AIA文件（匹配模式：{file_pattern}）：{all_files}")
    ...
    for file in all_files:
        ...
        try:
            ...
            if time_list and flux_list:
                ...
                print(f"成功加载 {file}，共 {len(time_list)} 条有效数据（在指定时间范围内）")
        except Exception as e:
            print(f"加载 {file} 失败: {str(e)}，已跳过")
    
    return all_aia_data

def plot_combined_data(sxr_data, hxi_data, aia_data_list, start_time, end_time, 
                      vline_times):
    """将所有数据绘制到同一图表，支持多条竖线"""
    ...
    for i, vtime in enumerate(vline_times):
        ax1.axvline(x=vtime, color='black', 
                   linestyle=line_styles[i % len(line_styles)], 
                   linewidth=1.5, alpha=0.8,
                   label=f'时间点{i+1}: {vtime.strftime("%H:%M:%S")}')
    
    # 1. 绘制SXR数据 (ax1)
    ax1.semilogy(sxr_data['times'], sxr_data['xrsa']['smoothed'], 
                 label='SXR 0.5-4.0 Å (平滑)', color='red')
    ax1.semilogy(sxr_data['times'], sxr_data['xrsb']['smoothed'], 
                 label='SXR 1.0-8.0 Å (平滑)', color='green')
    ...
    for i, (energy, counts) in enumerate(hxi_data['data'].items()):
        ax2.semilogy(hxi_data['times'], counts, 
                    label=f'HXI {energy}', color=colors[i], linestyle='--')
    ...
    ax3.plot(sxr_data['times'][1:], sxr_data['xrsa']['deriv'], 
             label='SXR 0.5-4.0 Å 导数', color='pink', linestyle='-.')
    ax3.plot(sxr_data['times'][1:], sxr_data['xrsb']['deriv'], 
             label='SXR 1.0-8.0 Å 导数', color='lightgreen', linestyle='-.')
    ...
    if aia_data_list:
        ...
        for i, aia_data in enumerate(aia_data_list):
            ...
            ax4.semilogy(aia_data['times'], aia_data['flux'], 
                        label=aia_data['label'], linestyle=ls, alpha=0.7)
    ...
    ax1.xaxis.set_major_locator(mdates.MinuteLocator(interval=5))  # 可根据时间范围调整间隔
    ax1.xaxis.set_minor_locator(mdates.MinuteLocator())
    ax1.xaxis.grid(True, which='both', linestyle='--', alpha=0.5)
    plt.xticks(rotation=45, ha='right')
    
    # 设置标题，包含时间范围信息
    plt.title(f'SXR-HXR-AIA ({start_time.strftime("%Y-%m-%d %H:%M")}至{end_time.strftime("%H:%M")})', 
...
def main():
    ...
    parser.add_argument('--sxr', type=str, 
                      default='<DATA_ROOT>/dn_xrsf-l2-flx1s_g16_d20240808_v2-2-0.nc',
                      help='SXR数据文件路径')
    parser.add_argument('--hxi', type=str, 
                      default='<DATA_ROOT>/hxi_qld_levq1_20240808_19_hly_v03.fits',
                      help='HXI数据文件路径')
    parser.add_argument('--aia-dir', type=str, 
                      default='<DATA_ROOT>/JSOCdata/All/Flux_data/',
                      help='AIA数据文件夹路径')
    parser.add_argument('--aia-patterns', type=str, nargs='+', default=['aia_304.csv', 'aia_1600.csv'],
                  help='AIA文件匹配模式列表（支持通配符，如"aia_*.csv"）')
    parser.add_argument('--start-time', type=str, default='2024-08-08T19:00:00',
                      help='起始时间（格式：YYYY-MM-DDTHH:MM:SS）')
    parser.add_argument('--end-time', type=str, default='2024-08-08T20:00:00',
                      help='结束时间（格式：YYYY-MM-DDTHH:MM:SS）')
    # 新增竖线时间参数，支持多个时间点
    parser.add_argument('--vline-times', type=str, nargs='+', 
    ...
    try:
        ...
        # 转换竖线时间列表
        vline_times = [datetime.fromisoformat(t.replace('T', ' ')) for t in args.vline_times]
        print(f"将绘制 {start_time} 至 {end_time} 的数据，包含竖线时间点: {[t.strftime('%H:%M:%S') for t in vline_times]}")
    except ValueError:
        print("时间格式错误，请使用YYYY-MM-DDTHH:MM:SS格式（例如：2024-08-08T19:00:00）")
        return
    ...
    print("绘制综合图像...")
    fig = plot_combined_data(sxr_data, hxi_data, aia_data_list, start_time, end_time,
                           vline_times)
