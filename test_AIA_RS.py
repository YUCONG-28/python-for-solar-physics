...
# ============================== 配置参数 ==============================
# AIA图像参数
aia_file = 'D:/spike_topping_type_III/20250124/All/94/1/aia.lev1_euv_12s.2025-01-24T033037Z.94.image_lev1.fits'
aia_vmin = 0.4  # 最小值（对数尺度）
aia_vmax = 6666  # 最大值（对数尺度）
aia_cmap = 'sdoaia94'  # 如果这个颜色映射不存在，使用'hot'
aia_norm = colors.LogNorm(vmin=aia_vmin, vmax=aia_vmax)

# 射电图像参数
radio_file = r'D:\spike_topping_type_III\20250124\share\Data\UnPack\20250124UT0447-0450\ImageData_RRLL\149MHz\RR\149MHz_2025124_044910_449.fits'
radio_cmap = 'gist_heat'
...
# 等高线参数
contour_levels = [0.1, 0.5, 0.9]  # 等高线强度级别（百分比）
...
def plot_aia_image_simple(ax, aia_data, extent, title="AIA 94Å Image"):
    ...
    im = ax.imshow(aia_data, extent=extent, origin='lower',
                  cmap=cmap_to_use, norm=aia_norm, aspect='equal')
...
def main():
    ...
    print(f"显示范围: X=[{final_display_extent[0]:.0f}, {final_display_extent[1]:.0f}], "
    ...
    try:
        ...
        aia_crpix1, aia_crpix2, aia_crval1, aia_crval2, aia_cdelt1, aia_cdelt2, aia_rsun = get_sun_center_and_radius(aia_header)
        ...
        print(f"  AIA图像范围: X={aia_x_extent[0]:.0f}~{aia_x_extent[1]:.0f}, Y={aia_y_extent[0]:.0f}~{aia_y_extent[1]:.0f}")
        
        # 完整的AIA图像范围
        aia_full_extent = [aia_x_extent[0], aia_x_extent[1], aia_y_extent[0], aia_y_extent[1]]
    ...
    try:
        ...
        # 获取射电图像的太阳信息
        radio_crpix1, radio_crpix2, radio_crval1, radio_crval2, radio_cdelt1, radio_cdelt2, radio_rsun = get_sun_center_and_radius(radio_header)
        ...
        print(f"  射电图像范围: X={radio_x_extent[0]:.0f}~{radio_x_extent[1]:.0f}, Y={radio_y_extent[0]:.0f}~{radio_y_extent[1]:.0f}")
        
        # 完整的射电图像范围
        radio_full_extent = [radio_x_extent[0], radio_x_extent[1], radio_y_extent[0], radio_y_extent[1]]
        ...
        if np.any(radio_data_smoothed > 0):
            ...
            if np.any(positive_mask):
                ...
                if radio_range_val > 0:
                    contour_values = [radio_min + level * radio_range_val for level in contour_levels]
    ...
    try:
        ...
        time_str = base_name.split('.')[2] if len(base_name.split('.')) > 2 else base_name
        fig.suptitle(f'AIA + Radio Overlay\nAIA: {time_str} | Radio: {date_obs} {freq}MHz {polar}', 
                    fontsize=16, fontweight='bold')
        
        # 显示完整的AIA图像作为背景
        im = plot_aia_image_simple(ax, aia_map.data, aia_full_extent, 'AIA + Radio Contours')
        ...
        print(f"  缩放后射电图像范围: X={scaled_radio_extent[0]:.0f}~{scaled_radio_extent[1]:.0f}, "
            ...
            if len(valid_contour_indices) > 0:
                ...
                for i in valid_contour_indices:
                    ...
                    if level > 0:  # 再次检查确保level是正值
                        try:
                            # 创建等高线
                            contour = ax.contour(radio_data_smoothed, levels=[level], 
                                                 colors=color, linewidths=lw, 
                                                 alpha=contour_alpha, 
                                                 extent=scaled_radio_extent, origin='lower')
                            ...
                            if i == 1:  # 只在50%等高线上添加标签
                                ...
                                ax.clabel(contour, inline=True, fontsize=9, fmt=make_label)
                        except Exception as e:
                            print(f"  绘制等高线 {contour_levels[i]*100:.0f}% 时出错: {e}")
        
        if not has_valid_contours:
            print("  警告: 没有有效的等高线可绘制")
        
        # 添加太阳轮廓（使用AIA太阳半径）
        solar_limb = patches.Circle((0, 0), radius=aia_rsun, 
        ...
        print(f"  最终显示范围: X=[{final_display_extent[0]:.0f}, {final_display_extent[1]:.0f}], "
              f"Y=[{final_display_extent[2]:.0f}, {final_display_extent[3]:.0f}] arcsec")
        ...
        ax.annotate('N', xy=(north_x, north_y), xytext=(north_x, north_y + y_range * 0.1),
                   ha='center', va='bottom', fontsize=12, color='yellow',
                   arrowprops=dict(arrowstyle='->', color='yellow', lw=1.5))
        
        # 东方向标记
        east_x = final_display_extent[1] - x_range * 0.05
        east_y = y_center
        ax.annotate('E', xy=(east_x, east_y), xytext=(east_x + x_range * 0.1, east_y),
                   ha='left', va='center', fontsize=12, color='yellow',
                   arrowprops=dict(arrowstyle='->', color='yellow', lw=1.5))
        
        # 添加图例
        from matplotlib.lines import Line2D
        legend_elements = [
            Line2D([0], [0], color='cyan', lw=2, linestyle='-', label=f'AIA Solar Limb (R={aia_rsun:.0f}")'),
        ]
        ...
        if has_valid_contours:
            for i in valid_contour_indices:
                legend_elements.append(
                    Line2D([0], [0], color=contour_colors[i], lw=contour_linewidths[i],
        ...
        ax.text(0.02, 0.02, range_text, transform=ax.transAxes, 
                fontsize=9, color='white', 
                bbox=dict(boxstyle="round,pad=0.3", facecolor="black", alpha=0.7))
        
        # 添加太阳中心标记
        ax.scatter(0, 0, color='white', s=50, marker='+', linewidth=1.5, label='Solar Center')
