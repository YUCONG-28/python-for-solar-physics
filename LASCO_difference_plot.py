...
# 配置参数（请根据实际情况修改）
# --------------------------
input_dir = '<DATA_ROOT>/data/'    # 输入数据目录
output_dir = '<DATA_ROOT>/difference_plot/'  # 输出图像目录
vmin = -49    # 颜色映射最小值
vmax = 49     # 颜色映射最大值
...
def get_jp2_files(input_dir, recursive=True):
    ...
    # 构建匹配模式：大小写不敏感的.jp2后缀
    if recursive:
        # 递归查找所有子目录
        pattern = os.path.join(input_dir, '**', '*.[jJ][pP]2')
    else:
        # 仅查找当前目录
        pattern = os.path.join(input_dir, '*.[jJ][pP]2')
    
    # 获取所有匹配文件（递归需设置recursive=True）
    files = glob.glob(pattern, recursive=recursive)
    
    # 按文件名中的时间戳排序（确保顺序正确）
    files_sorted = sorted(files, key=lambda x: extract_timestamp_from_filename(os.path.basename(x)))
...
def main():
    ...
    for idx, file in enumerate(files, 1):
        print(f"{idx:2d}. {os.path.basename(file)}")
    print("=" * 50)
    ...
    for i in range(1, len(files)):
        ...
        curr_basename = os.path.basename(curr_file)
        ...
        try:
            ...
            if prev_map.data.shape != curr_map.data.shape:
                print(f"警告：{prev_basename}与{curr_basename}维度不匹配（{prev_map.data.shape} vs {curr_map.data.shape}），跳过")
                continue
            ...
            # 构建输出文件名（保留时间戳，便于追溯）
            curr_base = os.path.splitext(curr_basename)[0]
            output_path = os.path.join(output_dir, f"diff_bw_{curr_base}.png")
            ...
            print(f"错误：处理{prev_basename}与{curr_basename}时失败 → {type(e).__name__}: {str(e)}")
            continue
