...
def get_sorted_fits_files(input_dir: str, min_size_kb: int = 1) -> List[Tuple[str, datetime.datetime]]:
    """
    获取目录下所有FITS文件并按时间排序
    ...
    Returns:
        List[Tuple[str, datetime.datetime]]: 排序后的文件列表，每个元素为(文件路径, 时间)
    """
    files = []
        ...
        if f.lower().endswith('.fits'):
            try:
                ...
                if os.path.getsize(file_path) < min_size_kb * 1024:
                    warnings.warn(f"跳过空/小文件：{f}")
                    continue
                file_time = extract_time_from_filename(f)
                files.append((file_path, file_time))
            except ValueError as e:
                warnings.warn(f"跳过文件（时间提取失败：{e}）：{f}")
            except Exception as e:
                warnings.warn(f"处理文件时出错：{f}，错误：{e}")
    
    return sorted(files, key=lambda x: x[1])


def find_closest_file_by_time(target_time: datetime.datetime, 
                             file_list: List[Tuple[str, datetime.datetime]], 
                             max_diff_seconds: float = 3600) -> Optional[Tuple[str, datetime.datetime]]:
    """
    ...
    closest_file = min(file_list, key=lambda x: abs((x[1] - target_time).total_seconds()))
...
def filter_files_by_time_range(file_list: List[Tuple[str, datetime.datetime]],
                              start_time: datetime.datetime,
                              end_time: datetime.datetime) -> List[Tuple[str, datetime.datetime]]:
    """
    按时间范围筛选文件
...
def monitor_memory_usage(description: str = "") -> Dict[str, float]:
    ...
    try:
        ...
        if description:
            print(f"{description}: RSS={result['rss_mb']:.1f}MB, "
...
def create_aia_submap(aia_map: sunpy.map.GenericMap, 
                     roi_bounds: Tuple[float, float, float, float]) -> sunpy.map.GenericMap:
    """
    创建AIA数据的子图区域
...
def align_maps_to_reference(source_map: sunpy.map.GenericMap, 
                           target_wcs) -> sunpy.map.GenericMap:
    """
    将源地图重投影到目标坐标系
...
def process_hmi_magnetic_field(hmi_map: sunpy.map.GenericMap,
                              threshold: u.Quantity = 0 * u.Gauss,
                              sigma: float = 3) -> sunpy.map.GenericMap:
    """
    处理HMI磁场数据：应用阈值和高斯滤波
...
def create_magnetic_contour_levels(base_level: u.Quantity = 50 * u.Gauss) -> u.Quantity:
    """
    创建磁场等值线层级
    ...
    Returns:
        u.Quantity: 等值线层级数组
    """
    levels = [base_level] * u.Gauss
    return np.concatenate((-1 * levels[::-1], levels))
...
def add_frequency_highlight_lines(ax, frequencies: List[float], 
...
def validate_time_range(start_time: datetime.datetime, 
