...
try:
    ...
    roi_bottom_left = SkyCoord(Tx=180 * u.arcsec, Ty=-340 * u.arcsec, frame=base_normalized_map.coordinate_frame)
    roi_top_right = SkyCoord(Tx=520 * u.arcsec, Ty=20 * u.arcsec, frame=base_normalized_map.coordinate_frame)
    base_cutout_map = base_normalized_map.submap(roi_bottom_left, top_right=roi_top_right)
    ...
    base_name = os.path.basename(sliced_files[0])
...
for i in tqdm(range(1, total_files), desc="Processing", unit="File"):
    try:
        ...
        output_path = os.path.join(output_dir, f"{current_name}_diff_from_base.png")
        ...
        del current_map, current_normalized_data, current_normalized_map, current_cutout_map
