...
`docs/quickstart.md`.
For the fused Radio module inventory and selective `/radio` execution contract,
see `docs/radio_workspace.md`.
...
| `solar-webapp` | Local English workflow workbench plus the integrated `/radio` workspace. | Radio modules share one port and run only explicit actions; source-only non-radio recipes are unavailable when absent from the installed package. |
...
| main | `scripts/radio/run_radio_roi_lightcurve_app.py` | Streamlit frontend for discovering radio frequencies from a FITS folder, loading selected bands, selecting files by stable File # values or ranges, rendering synchronized multi-frequency reference images, confirming one box or lasso ROI, previewing the light curve, and exporting selected products. Step 3 reference controls are submitted together through `Render Reference Grid`; identical requests reuse cached reference images, and later display or ROI changes do not reread FITS. | Radio FITS folder, frequency bands, optional data-derived time filters, selected FITS files, reference anchor File #, HPLN/HPLT ROI selection, display range/colormap/FOV controls, polarization mode, metric, and output folder. | Selectable ROI JSON with display/reference metadata, raw statistics CSV, multi-frequency reference PNG, and light-curve PNG with `raw_sum`, `raw_mean`, and `raw_peak` statistics. |
| utility | `scripts/radio/run_rrll_percentile_preview_comparison.py` | Focused 2025-01-24 RR+LL nine-band batch runner that compares four fixed per-band percentile ranges at the first, middle, and last event slots. | Event radio FITS/config data or explicit `--radio-root` and `--spectrogram-file` overrides, output directory, optional run stem, and optional unique run tag. | Twelve source-map preview PNGs, per-preview metadata/provenance JSON files, and a run summary JSON. |
...
| utility | `scripts/tools/run_solar_webapp.py` | Launch the unified local English web GUI and its selective `/radio` workspace. The Radio sidebar fuses overlapping workflows into eight research-purpose modules; enablement, collapse, ordering, and presets do not start jobs. | Allowed-root boundary, optional Radio output root, and workbench port. | Single-port dashboard with independent Preview/Run/Cancel actions, confirmed Run Selected batches, persisted layouts, queue/log/provenance, and safe artifact previews/downloads. |
...
| utility | `solar_toolkit/radio/roi_lightcurve.py` | HPLN/HPLT box or polygon ROI masking, raw radio FITS statistics, explicit selected-file extraction, LCP/RCP pairing, and in-memory or on-disk CSV/JSON/PNG product writing. | `scripts/radio/run_radio_roi_lightcurve_app.py` and `solar-radio roi-lightcurve`. |
...
| utility | `solar_toolkit/radio/roi_selection_cli.py` | Convert same-page Plotly box/lasso selections or existing ROI JSON into the canonical persisted ROI contract. | Radio Workspace `Select ROI`; no light-curve or upstream action is started. |
| utility | `solar_toolkit/radio/drift_selection_cli.py` | Convert explicit two-endpoint drift lines into canonical drift-rate JSON and diagnostic CSV products. | Radio Workspace `Select Drift Lines`; accepts an existing spectrogram preview and metadata. |
| utility | `solar_toolkit/radio/trajectory_media_cli.py` | Filter compatible center tables and create reproducible MP4/GIF/WebM trajectory media. | Radio Workspace `Interactive Trajectory & Media`; independent of center extraction. |
| utility | `solar_toolkit/radio/physical_diagnostics_cli.py` | Build Newkirk heights, residuals, drift speeds, frequency summaries, a consistency report, and dashboard products from existing Gaussian and/or drift tables. | Radio Workspace `Analyze Existing Tables`; no imaging or spectrogram action is started. |
| utility | `solar_toolkit/radio/existing_fit_overlay.py`, `existing_fit_overlay_cli.py` | Normalize existing center and/or Gaussian CSV products, optionally read one AIA FITS background, and write a static overlay PNG plus metadata JSON without running a fit. | Radio Workspace `Overlay Existing Centers and Fits`; each persisted artifact binds to an explicit path field. |
| utility | `solar_toolkit/xray_dem/dem_radio_cli.py` | Render a typed DEM/Tb-radio comparison from an AIA reference FITS, Tb NPY map, and either one radio FITS or a time-matched radio folder. | Radio Workspace `DEM/Radio Overlay` under Context / Adjacent; no interactive prompt or upstream computation is required. |
...
| utility | `solar_toolkit/webapp/radio_workspace/` | Versioned eight-module catalog, workspace/run/artifact contracts, allowed-root store, native ROI/drift/trajectory previews, selected-action runner, and `/api/radio/*` Blueprint. | Integrated `/radio` UI, reusable artifacts with source run IDs, 1-4 workspace concurrency, cancellation, and interrupted-run recovery. |
