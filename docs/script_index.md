...
| main | `scripts/radio/run_radio_roi_lightcurve_app.py` | Streamlit frontend for loading a radio FITS folder, selecting one or more data files, rendering a reference image, confirming a box or lasso ROI, previewing the light curve, and exporting selected products. | Radio FITS folder, optional time filters, selected FITS files, reference image, HPLN/HPLT ROI selection, polarization mode, metric, and output folder. | Selectable ROI JSON, raw statistics CSV, reference PNG, and light-curve PNG with `raw_sum`, `raw_mean`, and `raw_peak` statistics. |
