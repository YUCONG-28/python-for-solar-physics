...
| main | `scripts/radio/radio_source_map_plot_gaussian_overlay.py` | Advanced radio source map workflow with RR/LL handling, multi-band panels, Gaussian fitting, CSO spectrogram panels, and manual drift-rate overlays. | Radio source FITS files, optional CSO spectrogram FITS, Gaussian/drift settings. | Radio source maps, fitted centers, FWHM overlays, diagnostics CSV, optional drift-rate JSON/preview products. |
| main | `scripts/radio/sdo_aia_radio_hmi_overlay.py` | Overlay radio source contours and optional HMI contours on AIA context images. | AIA FITS, radio source FITS, optional HMI FITS, matching and fit settings. | AIA-radio or AIA-radio-HMI diagnostic figures with contours and fitted source markers. |
| main | `scripts/radio/cso_radio_spectrogram_plot.py` | Plot CSO dynamic spectra with memory-aware slicing and downsampling. | CSO spectrogram FITS, time/frequency ranges, polarization settings. | LL/RR, total intensity, and polarization-ratio spectrum figures. |
