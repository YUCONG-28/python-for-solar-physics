...
| Radio Workspace contracts/services | `solar_toolkit.webapp.radio_workspace.contracts`, `catalog`, `store`, `native_previews` | Versioned module/action/workspace/run/artifact schemas, allowed-root persistence, and native previews |
| Radio Workspace orchestration | `solar_toolkit.webapp.radio_workspace.runner`, `api` | Selected-action queue, cancellation, provenance, artifact reuse, and `/api/radio/*`; never auto-runs upstream modules |
