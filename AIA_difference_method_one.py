...
if len(sliced_files) < 2:
    raise ValueError(f"选取的文件范围需要至少2个文件，当前仅找到{len(sliced_files)}个文件!")
...
try:
    ...
    roi_bottom_left = SkyCoord(Tx=600 * u.arcsec, Ty=-280 * u.arcsec, frame=first_normalized_map.coordinate_frame)
    roi_top_right = SkyCoord(Tx=1210 * u.arcsec, Ty=100 * u.arcsec, frame=first_normalized_map.coordinate_frame)
    first_cutout_map = first_normalized_map.submap(roi_bottom_left, top_right=roi_top_right)
    ...
    first_name = os.path.basename(sliced_files[0])
    output_path = os.path.join(output_dir, f"{first_name}.png")
...
for i in tqdm(range(1, total_files), desc="Processing", unit="File"):
    try:
        ...
        prev_cutout_map = prev_normalized_map.submap(roi_bottom_left, top_right=roi_top_right)
        ...
        current_normalized_map = sunpy.map.Map(current_normalized_data, current_map.meta)
        current_cutout_map = current_normalized_map.submap(roi_bottom_left, top_right=roi_top_right)
        ...
        output_path = os.path.join(output_dir, f"{current_name}_diff.png")
        prev_time_str = os.path.basename(prev_file).split('.')[2]
