...
def filter_and_copy_aia_files(src_base_dir, out_base_dir, target_time_str, tolerance_seconds=15):
    # ================= 核心逻辑处理 =================
    # 1. 解析目标中心时间
    ...
    target_base_dir = os.path.join(out_base_dir, time_suffix)

    target_aia = os.path.join(target_base_dir, "AIA")
    target_rasis = os.path.join(target_base_dir, "RaSIS")
    target_rs = os.path.join(target_base_dir, "RS")
    ...
    print("\n开始扫描并收集时间窗口内的文件...")
    for root, _, files in os.walk(src_base_dir):
        for file in files:
            if file.endswith(".fits"):
                ...
                if match:
                    ...
                    if dt_start <= file_dt <= dt_end:
                        ...
                        candidates[wave].append((src_file_path, file, file_dt))
    ...
    if not candidates:
        print("警告：在指定的时间窗口内未找到任何符合条件的 AIA 文件，请检查时间或调大 tolerance_seconds。")
        return

    for wave, file_list in candidates.items():
        best_file = min(file_list, key=lambda x: abs((x[2] - target_time).total_seconds()))
        
        src_file_path = best_file[0]
        file_name = best_file[1]
        dst_file_path = os.path.join(target_aia, file_name)

        if not os.path.exists(dst_file_path):
            shutil.copy2(src_file_path, dst_file_path)
            print(f"波段 {wave:>4} | 选定时间: {best_file[2].strftime('%H:%M:%S')} | 成功复制: {file_name}")
            copied_count += 1
        else:
...
if __name__ == "__main__":
    ...
    SOURCE_DIR = r"<PROJECT_ROOT>\2025\20250124\All"
    OUTPUT_BASE_DIR = r"<PROJECT_ROOT>\2025\20250124\DEM"
