...
def load_single_file(file_path: str) -> Tuple[List[datetime], List[float], str]:
    ...
    try:
        ...
        return time_list, flux_list, os.path.basename(file_path)
...
def plot_multi_data(all_data: List[Tuple[List[datetime], List[float], str]], output_dir: str, file_names: List[str], mark_times: List[datetime] = None):
    ...
    for time_list, flux_list, label in all_data:
        ...
        plt.plot(time_list, flux_list, linestyle=ls, color=color, 
    ...
    if mark_times:
        for mark_time in mark_times:
            ...
            if x_min <= mark_time <= x_max:
                plt.axvline(x=mark_time, color='purple', linestyle='--', alpha=0.7, linewidth=1.5, 
    ...
    output_path = os.path.join(output_dir, 'The postflareloop region.png')
...
def main():
    # 解析命令行参数（修改为按文件名选取）
    parser = argparse.ArgumentParser(description='从自定义文件夹读取指定文件名的CSV文件并绘制对比曲线')
    parser.add_argument('--dir', type=str, default='<DATA_ROOT>/JSOCdata/All/Flux_data/',
                      help='数据文件所在的文件夹路径（例如：D:/my_data/）')
    parser.add_argument('--output', type=str, default='<DATA_ROOT>/JSOCdata/All/Flux/',
                      help='图像输出文件夹路径（默认与输入文件夹相同）')
    parser.add_argument('--files', type=str, nargs='+', 
                    default=['aia_94_selected_2.csv','aia_131_selected_2.csv','aia_171_selected_2.csv','aia_193_selected_2.csv','aia_211_selected_2.csv','aia_304_selected_2.csv','aia_1600_selected_2.csv'],  # 设置默认文件列表
                    required=False,  # 可选参数（因为有默认值）
                    help='要选取的CSV文件名列表（例如：file1.csv file2.csv file3.csv）')
    parser.add_argument('--times', type=str, nargs='*', default=['2024-08-08 19:22:30','2024-08-08 19:27:30'],
    ...
    if args.times:
        for time_str in args.times:
            try:
                ...
                print(f"警告：时间格式错误 '{time_str}'，请使用YYYY-MM-DD HH:MM:SS格式，已跳过")
    
    # 获取文件夹中所有CSV文件（用于验证）
    all_csv_files = [f for f in os.listdir(input_dir) 
