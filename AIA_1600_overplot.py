...
output_dir = 'D:/Flare/JSOCdata/AIA_1600/plot_overlap'
...
if len(png_files) > 0:
    # 打开第一张图片
    base_image = Image.open(png_files[0]).convert("RGBA")
    for png_file in png_files[1:]:
        # 打开后续图片
        overlay_image = Image.open(png_file).convert("RGBA")
        # 叠加图片
        base_image = Image.alpha_composite(base_image, overlay_image)
    
    # 保存叠加后的图片
    overlay_output_path = os.path.join(output_dir, "overlay.png")
    base_image.save(overlay_output_path)