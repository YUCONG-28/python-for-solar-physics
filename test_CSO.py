...
class spectrogram:
    def __init__(self, data=None, time=None, freq=None, polar=None, dateobs=None, unit=None, obsdev=None, dt_base=None):
...
def readcso_spectrofits(fn):
    ...
    try:
        ...
        dataout = []
        if data.ndim == 2:
            dataout.append(spectrogram(
                data=data, time=time, freq=freq,
                polar=polars, dateobs=dateobs, unit=unit, dt_base=datet0
            ))
            ...
            for ii in tqdm(range(arr_n), desc="读取偏振数据"):
                polar = polars[ii] * 2
                dataout.append(spectrogram(
                    data=data[ii, :, :], time=time, freq=freq,
                    polar=polar, dateobs=dateobs, unit=unit, dt_base=datet0
                ))
...
@timing_decorator
def process_and_plot_data(data_list, t_start, t_end, f_start, f_end, rebin_factor,
                          plot_ll=True, plot_rr=True, plot_ratio=True, plot_sum=True):
    """
    ...
    for idx, item in enumerate(plot_items):
        ax = axs[idx]
        im = ax.pcolormesh(xx, yy, item['data'][:-1, :-1], shading='flat',
                           cmap=item['cmap'], vmin=item['vmin'], vmax=item['vmax'])
...
if __name__ == '__main__':
    ...
    process_and_plot_data(data_list, t_start, t_end, f_start, f_end, rebin_factor,
                           plot_ll=False, plot_rr=False, plot_ratio=True, plot_sum=True)
