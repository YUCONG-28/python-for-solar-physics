    ...
    with (
        ...
        for index in range(0, min(len(aia_sequence), len(hdul))):
            ...
            for offset in range(4):
                ...
                try:
                    wavelength = int(round(aia_submap.wavelength.to_value(u.Angstrom)))
                except Exception:
                    wavelength = 304
                output_path = output_dir / build_scientific_image_filename(
                    sequence=4 * index + offset + 1,
                    start_time=getattr(aia_submap, "date", None),
                    instrument="aia_hxi",
                    channel=f"{wavelength}a",
                    product="contour_overlay",
