...
def render_dem_radio_overlay(
    ...
    with _CONFIG_LOCK:
        ...
        try:
            ...
            image_path = request.output_dir / build_scientific_image_filename(
                sequence=1,
                start_time=getattr(aia_map, "obs_dt", None) or aia_map.obs_time,
                instrument="aia_dem_radio",
                product="brightness_temperature_overlay",
                generated_at=datetime.now(timezone.utc),
            )
            workflow.CONFIG["output_filename"] = str(image_path)
