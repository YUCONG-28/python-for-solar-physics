"""Local workbench command adapter with a fail-closed root policy."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="Apps/run.ps1 frontend workbench",
        description="Run the local Solar Physics Workbench.",
        epilog="Additional arguments are forwarded to the workbench parser.",
    )
    parser.add_argument(
        "--allowed-roots",
        metavar="PATHS",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"webapp: error: {exc}", file=sys.stderr)
        return 2
