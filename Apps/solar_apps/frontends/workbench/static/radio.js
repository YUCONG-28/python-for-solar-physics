(function radioWorkspaceApp() {
  ...
  function scheduleLayoutSave() {
    ...
    if (!state.workspace) {
      window.SolarUiState?.saveFields("workbench", {
        "radio-workspace.layout": JSON.stringify(state.layout),
      });
      return;
    }
  ...
  function renderField(field) {
    ...
    if (field.path) {
      ...
      browse.addEventListener("click", () => openFileBrowser(input, field));
  ...
  async function previewAction(module, action, card) {
    ...
    try {
      if (action.preview_adapter === "file-browser") {
        ...
        return openFileBrowser(input, {path_kind: "directory", extensions: [], allow_multiple: false});
      }
  ...
  async function saveWorkspace(event) {
    ...
    try {
      ...
      window.SolarUiState?.saveFields("workbench", {
        "radio-workspace.workspace-id": state.workspace.id,
      });
  ...
  async function openFileBrowser(target, field = {path_kind: "file"}) {
    if (!target) return;
    const spec = typeof field === "string" ? {path_kind: field} : (field || {});
    const pathKind = spec.path_kind || "file";
    const mode = pathKind === "directory"
      ? "select_directory"
      : pathKind === "save_file"
        ? "save_file"
        : spec.allow_multiple
    ...
    try {
      const paths = await window.SolarNativePathDialog.select({
        mode,
        initialPath: target.value || "",
        title: spec.label || (pathKind === "directory" ? "Select local folder" : "Select local file"),
        extensions: spec.extensions || [],
        defaultSuffix: spec.default_suffix || "",
        operation: target.closest(".action-card")?.dataset.actionId || "configure-workspace",
        field: spec.name || target.name || target.id || "path",
      });
      if (!paths.length) {
        showNotice("Path selection cancelled.", "warning");
        return;
      }
      target.value = spec.allow_multiple
        ? window.SolarNativePathDialog.appendUniqueLines(target.value, paths)
        : paths[0];
      target.dispatchEvent(new Event("change", {bubbles: true}));
      showNotice("Local path selected.", "success");
    } catch (error) {
      showNotice(`Native path dialog failed: ${error.message}`, "error");
    }
  }
  ...
  function renderArtifact(run, artifact, siblingArtifacts = []) {
    ...
    download.setAttribute(
      "download",
      artifact.suggested_filename || artifact.relative_path.split("/").pop() || "artifact",
    );
  ...
  async function switchWorkspace(workspaceId) {
    ...
    state.pollTimer = null;
    window.SolarUiState?.saveFields("workbench", {
      "radio-workspace.workspace-id": workspaceId,
    });
    populateWorkspaceChoices();
    getFigureComposer().setWorkspace(state.workspace);
    renderAll();
  ...
  async function loadExistingWorkspace(rememberedWorkspaceId = "") {
    const payload = await requestJson(`${API}/workspaces`);
    const workspaces = payload.workspaces || [];
    state.workspaces = workspaces;
    state.workspace = workspaces.find((item) => item.id === rememberedWorkspaceId) || workspaces[0] || null;
  ...
  async function boot() {
    wireEvents();
    const savedUiState = await (window.radioUiStatePromise || Promise.resolve({fields: {}}));
    const savedUiFields = savedUiState?.fields || {};
    ...
    try {
      ...
      try {
        await loadExistingWorkspace(String(savedUiFields["radio-workspace.workspace-id"] || ""));
      } catch (error) {
        showNotice(`Saved workspaces could not be opened: ${error.message}`);
      }
      let localLayout = null;
      try { localLayout = JSON.parse(savedUiFields["radio-workspace.layout"] || "null"); } catch { localLayout = null; }
