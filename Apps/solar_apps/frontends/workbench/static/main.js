...
const els = {
  ...
  pathBrowseStatus: document.getElementById("pathBrowseStatus"),
...
function visibleModules() {
  ...
  return state.modules.filter((item) =>
    [item.title, item.category, item.description, item.workflow_id, item.command_module]
      .join(" ")
      .toLowerCase()
      .includes(query)
  );
...
async function selectModule(moduleId) {
  ...
  els.moduleDetail.innerHTML = `<h2>${escapeHtml(payload.module.title)}</h2>
    ...
    <dl>
      ...
      <dt>Workflow ID</dt><dd><code>${escapeHtml(payload.module.workflow_id)}</code></dd>
      <dt>Command module</dt><dd><code>${escapeHtml(payload.module.command_module)}</code></dd>
...
async function addLocalPaths(mode) {
  els.pathBrowseStatus.textContent = "Opening Windows dialog...";
  try {
    const paths = await window.SolarNativePathDialog.select({
      mode,
      initialPath: els.pathsInput.value.split(/\r?\n/).filter(Boolean).at(-1) || "",
      title: mode === "select_directory" ? "Add local folder" : "Add local file(s)",
      operation: "run-feature",
      field: "paths-input",
    });
    if (paths.length) {
      els.pathsInput.value = window.SolarNativePathDialog.appendUniqueLines(
        els.pathsInput.value,
        paths,
      );
      els.pathBrowseStatus.textContent = `${paths.length} path(s) added.`;
    } else {
      els.pathBrowseStatus.textContent = "Selection cancelled.";
    }
  } catch (error) {
    els.pathBrowseStatus.textContent = error.message;
  }
}
...
function installHandlers() {
  ...
  els.addFilesBtn.addEventListener("click", () => addLocalPaths("open_files"));
  els.addFolderBtn.addEventListener("click", () => addLocalPaths("select_directory"));
