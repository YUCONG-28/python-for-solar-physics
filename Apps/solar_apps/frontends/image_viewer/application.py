"""Command-line launcher for the local image sequence web viewer.

English: Thin CLI for the Flask app factory in
`solar_apps.frontends.image_viewer`.

中文: 本文件只负责解析命令行参数并启动 Flask 服务, 具体扫描、显示和
导出逻辑位于 `solar_apps.frontends.image_viewer`。
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        from solar_apps.frontends.image_viewer.server import create_app
