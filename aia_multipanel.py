...
def organize_aia_files(data_directory):
    ...
    """
    search_pattern = os.path.join(data_directory,  '*.fits')
    ...
    for filepath in all_files:
        ...
        if match:
            ...
            classified_files[key].append(filepath)
...
def process_aia(outdir, file_list, log_scale=True, global_vmin=None, global_vmax=None, auto_range=True, percentile_range=(1, 99.9), return_stats=True):
    ...
    if auto_range and (global_vmin is None or global_vmax is None):
        ...
        for fil in file_list[::20]: 
            try:
                ...
                roi_bottom_left = SkyCoord(Tx=650 * u.arcsec, Ty=100 * u.arcsec, frame=current_map.coordinate_frame)
                roi_top_right = SkyCoord(Tx=1050 * u.arcsec, Ty=-300 * u.arcsec, frame=current_map.coordinate_frame)
                cropped_map = normalized_map.submap(roi_bottom_left, top_right=roi_top_right)
        ...
        if all_data:
            ...
            print(f"wave display range: vmin={calculated_vmin:.2f}, vmax={calculated_vmax:.2f}")
        else:
            calculated_vmin, calculated_vmax = None, None
...
def get_multi_wave_df(all_classified_files, target_waves=[94, 131, 171, 193, 211, 304]):
        ...
        for f in files:
            ...
            if match:
                ...
                records.append({
                    ...
                    'abs_sec': t_obj.unix                 })
        if records:
            wave_dfs[wl] = pd.DataFrame(records).sort_values('abs_sec').reset_index(drop=True)
        else:
    ...
    for idx, row in base_df.iterrows():
        ...
        for wl in target_waves:
            if wl == base_wave: continue
            
            df_target = wave_dfs.get(wl)
            if df_target is None or df_target.empty: 
                match_success = False; break
...
def plot_synced_aia(synced_data, outdir, wave_display_ranges, target_waves=[94, 131, 171, 193, 211, 304]):
    """
        ...
        try:
            ...
            for j, wl in enumerate(target_waves):
                ...
                if wl in item: 
                    ...
                    roi_bottom_left = SkyCoord(Tx=650 * u.arcsec, Ty=100 * u.arcsec, frame=m_norm.coordinate_frame)
                    roi_top_right = SkyCoord(Tx=1050 * u.arcsec, Ty=-300 * u.arcsec, frame=m_norm.coordinate_frame)
                    ...
                    vmax_val = wave_display_ranges.get(wl, {}).get('vmax', np.nanpercentile(m_crop.data[np.isfinite(m_crop.data)], 99.9)) 
                    
                    plot_data = m_crop.data.copy()
                    plot_data[plot_data <= 0] = np.nan
                    
                    im = m_crop.plot(axes=ax, title=f"", 
                                    norm=colors.LogNorm(vmin=vmin_val, vmax=vmax_val)
                                    ) 
                    precise_time = m_crop.date.strftime('%H:%M:%S')
                    label_text = f"{wl} Å | {precise_time}"

                    ax.text(0.03, 0.96, label_text, 
                    ...
                    ax.tick_params(axis='both', which='both', direction='in', 
                        color='white', labelsize=14, pad=2)
            ...
            fout_path = os.path.join(outdir, f"AIA_{time_str}.png")
            # plt.show()
            # plt.close()
            # break
...
if __name__ == '__main__':
    ...
    if wave_display_ranges is None:
        ...
        for wl in [94, 131, 171, 193, 211, 304]:
            if wl in all_classified_files:
                vmin, vmax = process_and_plot_aia(
                    outdir=None, 
    ...
    COMBO_OUT = os.path.expanduser('~/output/260105/jet/aia/') 
    plot_synced_aia(sync_data, COMBO_OUT,  wave_display_ranges,
